...
if { [string equal $gift_certificate_covers_cost_p "f"] } {
	...
	if { ![info exists creditcard_id] || ([info exists creditcard_number] && ![empty_string_p $creditcard_number]) } {
		...
		ad_returnredirect [export_vars -base checkout-2 { user_id participant_id }]
		ad_script_abort
	    }
	...
	if { $method == "internal_account" } {
		...
		ad_return_complaint 1 "<li> Please enter an internal account number.</li>"
		ad_script_abort		
	    }
...
if { [llength $assessment_ids] > 0 } {
    ...
    ad_returnredirect [export_vars -base [apm_package_url_from_key assessment]assessment { {assessment_id "[lindex $assessment_ids 0]"} {return_url $checkout_url} }]
    ad_script_abort
}
...
ad_returnredirect $checkout_url
ad_script_abort
